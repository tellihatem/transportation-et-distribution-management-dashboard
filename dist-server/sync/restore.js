    ...
    try {
      ...
      `);
            for (const t of trips) {
                insertTrip.run(t.id, t.date, t.client_name, t.origin_factory, t.destination, t.material_type, t.total_tonnage, t.truck_cost, t.driver_cut, t.company_profit, t.created_at || new Date().toISOString(), t.updated_at || new Date().toISOString());
            }
            // Insert resales
      ...
      `);
            for (const r of resales) {
                insertResale.run(r.id, r.date, r.end_client, r.factory_purchase_price, r.total_tonnage, r.client_selling_price, r.truck_cost, r.driver_cost, r.explicit_profit, r.created_at || new Date().toISOString(), r.updated_at || new Date().toISOString());
            }
            // Insert expenses
