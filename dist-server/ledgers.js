...
exports.allocateDriverPayment = allocateDriverPayment;
exports.applyClientCredit = applyClientCredit;
 ...
 */
function allocateClientPayment(paymentId, clientName, amount) {
...
function allocateDriverPayment(paymentId, driverName, amount) {
    let remaining = amount;
 ...
 */
function applyClientCredit(clientName) {
    if (!clientName)
        return;
...
function applyDriverAdvance(driverName) {
    if (!driverName)
        return;
 ...
 */
function reconcileWork(opts) {
    if (opts.tripId)
        releaseStaleAllocations(opts.tripType, opts.tripId);
    for (const name of new Set((opts.clientNames ?? []).filter(Boolean)))
