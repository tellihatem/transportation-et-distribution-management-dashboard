...
import { calcResale } from '../resale-math';
 ...
 */
function resaleSupplierCost(row: { factory_purchase_price: number; total_tonnage: number }): number {
  return calcResale({
 ...
 */
function syncTargetPaid(targetType: 'resale' | 'invoice', targetId: string) {
  const sumRow = db.prepare(`
    SELECT COALESCE(SUM(amount), 0) as total_allocated
    FROM supplier_payment_allocations
    WHERE target_type = ? AND target_id = ?
  `).get(targetType, targetId) as { total_allocated: number };
 ...
 */
function availableAdvance(supplierName: string): number {
  ...
  `).get(supplierName, supplierName) as { available: number };
...
function targetRemaining(
  ...
  if (targetType === 'resale') {
    const r = db.prepare(
      'SELECT * FROM material_resales WHERE id = ? AND origin_factory = ?'
    ).get(targetId, supplierName) as any;
    if (!r) return null;
    return resaleSupplierCost(r) - (r.supplier_paid ?? 0);
  }

  const i = db.prepare(
    'SELECT * FROM supplier_invoices WHERE id = ? AND supplier_name = ?'
  ).get(targetId, supplierName) as any;
  if (!i) return null;
...
function nextPaymentId(): string {
  const rows = db.prepare('SELECT id FROM supplier_payments').all() as { id: string }[];
...
function nextInvoiceId(): string {
  const rows = db.prepare('SELECT id FROM supplier_invoices').all() as { id: string }[];
  const max = rows.reduce((m, r) => {
 ...
 */
supplierPaymentsRouter.get('/', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
supplierPaymentsRouter.get('/next-id', asyncHandler(async (_req: Request, res: Response) => {
 ...
 */
supplierPaymentsRouter.get('/summary', asyncHandler(async (_req: Request, res: Response) => {
 ...
 */
supplierPaymentsRouter.get('/statement/:supplierName', asyncHandler(async (req: Request, res: Response) => {
  const name = req.params.supplierName;

  const resales = db.prepare('SELECT * FROM material_resales WHERE origin_factory = ? ORDER BY date DESC').all(name) as any[];
  const invoices = db.prepare('SELECT * FROM supplier_invoices WHERE supplier_name = ? ORDER BY date DESC').all(name) as any[];
  const payments = db.prepare('SELECT * FROM supplier_payments WHERE supplier_name = ? ORDER BY date DESC').all(name) as any[];
  ...
  res.json({
    ...
    data: {
      ...
      payments: payments.map(p => ({
        ...
        allocatedAmount: (db.prepare('SELECT COALESCE(SUM(amount), 0) as total FROM supplier_payment_allocations WHERE payment_id = ?').get(p.id) as any).total
      }))
    }
  });
}));
 ...
 */
supplierPaymentsRouter.post('/', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
function allocateFifo(paymentId: string, supplierName: string, amount: number): number {
  const unpaidResales = db.prepare(`
 ...
 */
supplierPaymentsRouter.put('/:id', asyncHandler(async (req: Request, res: Response) => {
  const paymentId = req.params.id;

  const existing = db.prepare('SELECT * FROM supplier_payments WHERE id = ?').get(paymentId) as any;
  ...
  const updateTx = db.transaction(() => {
    ...
    db.prepare('DELETE FROM supplier_payment_allocations WHERE payment_id = ?').run(paymentId);
 ...
 */
supplierPaymentsRouter.get('/available/:supplierName', asyncHandler(async (req: Request, res: Response) => {
 ...
 */
supplierPaymentsRouter.post('/deduct', asyncHandler(async (req: Request, res: Response) => {
  const { supplierName, targetType, targetId, amount } = req.body;

  if (!supplierName || !targetType || !targetId || !amount || amount <= 0) {
    throw createApiError('Missing required fields: supplierName, targetType, targetId, amount', 400, 'VALIDATION_ERROR');
  }
  if (targetType !== 'resale' && targetType !== 'invoice') {
    throw createApiError("targetType must be 'resale' or 'invoice'", 400, 'VALIDATION_ERROR');
  }

  const available = availableAdvance(supplierName);
  if (amount > available) {
    throw createApiError(
      `Deduction exceeds the advance on account (available: ${available})`,
      400,
  ...
  if (remaining === null) {
    throw createApiError('Shipment or invoice not found for this supplier', 404, 'NOT_FOUND');
  }
  if (amount > remaining) {
    throw createApiError(
      `Deduction exceeds what this shipment still owes (remaining: ${remaining})`,
      400,
      'EXCEEDS_REMAINING'
    );
  }
 ...
 */
supplierPaymentsRouter.delete('/:id', asyncHandler(async (req: Request, res: Response) => {
  const paymentId = req.params.id;
 ...
 */
supplierInvoicesRouter.get('/', asyncHandler(async (req: Request, res: Response) => {
  const { supplierName, dateStart, dateEnd } = req.query;
 ...
 */
supplierInvoicesRouter.get('/next-id', asyncHandler(async (_req: Request, res: Response) => {
 ...
 */
supplierInvoicesRouter.post('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  const existing = db.prepare('SELECT id FROM supplier_invoices WHERE id = ?').get(id);
  if (existing) throw createApiError('Invoice ID already exists', 409, 'DUPLICATE_ID');
  ...
  const created = db.prepare('SELECT * FROM supplier_invoices WHERE id = ?').get(id);
  res.status(201).json({ success: true, data: created });
}));
 ...
 */
supplierInvoicesRouter.delete('/:id', asyncHandler(async (req: Request, res: Response) => {
  const invoiceId = req.params.id;

  const existing = db.prepare('SELECT * FROM supplier_invoices WHERE id = ?').get(invoiceId);
